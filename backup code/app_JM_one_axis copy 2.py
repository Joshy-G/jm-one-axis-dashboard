...
def load_and_process_trade_data():
    ...
    raw_df["Equity"] = INITIAL_CAPITAL + raw_df["P&L"].cumsum()
...
if df.empty:
    # No trades at all – capital = initial
    start_capital = INITIAL_CAPITAL
    close_capital = INITIAL_CAPITAL
else:
    # Get the last equity value before today (if any)
    ...
    # Closing capital = last equity value overall (could be today or earlier)
...
if menu == "Trade Entry":
    ...
    if st.button("Submit Trade") and not st.session_state.trade_submitted:
        ...
        trade_id = f"JM-{now.strftime('%Y%m%d')}-{trade_count:03d}"
